...
class CreateCellClusters(SteppableBasePy):
    ...
    def start(self):
        ...
        for cell in self.cell_list_by_type(self.MHP_LATERAL, self.NP_LATERAL_L, self.NP_LATERAL_R):
            for cell2 in self.cell_list_by_type(self.MHP_APICAL, self.MHP_BASAL, self.NP_BASAL_L, self.NP_BASAL_R, self.NP_APICAL_L, self.NP_APICAL_R):
...
class NeuraltubeModelCellBehaviors(SteppableBasePy):
    ...
    def add_mesoderm_matrix_cell(self, cell):
        if cell.type in [MESODERM_LEFT, MESODERM_RIGHT]:
            ...
            pixelTrackerDataList=CellBoundaryPixelList(self.boundaryPixelTrackerPlugin, cell)    #careful, not really a python list
            pixelList=[]
    ...
    def calculate_outermost_cell_distance(self):
        ...
        leftmost_cell_l = [cell for cell in self.cell_list_by_type(self.NP_LATERAL_R) if cell.dict["OUTERMOST"] == "LEFT"] 
        rightmost_cell_l = [cell for cell in self.cell_list_by_type(self.NP_LATERAL_R) if cell.dict["OUTERMOST"] == "RIGHT"] 
    ...
    def start(self):
        ...
        for cell in self.cellList:
            ...
            self.orientedGrowthPlugin.setElongationEnabled(cell, False) # Make sure to enable or disable elongation in ALL cells  
        ...
        for cell in self.cellList: ## This is the main loop through all the cells ##
           ...
           # e = self.isBoundaryCell(cell, [ENDODERM])
            ...
            if mcs <= self.time_to_stop_ectodermal_growth:            
                if cell.type in [ECTODERM_LEFT, ECTODERM_RIGHT]:
                    growth_rate = self.ectoderm_growth_rate + random.uniform (-self.ectoderm_growth_rate, self.ectoderm_growth_rate)
                    cell.targetVolume += growth_rate
            ...
            if self.enable_apical_constriction_behavior:
                if mcs >= 3000: #start apical constriction later in time
                    ...
                    self.set_behaviour_flag_apical_constriction_BMP4(# set ENABLE_CONSTRICT dict flag to True / False depending on BMP4 concentration
                    cell, 
                    field = BMP4_field,
                    ...
                    self.set_behaviour_flag_apical_constriction_SHH(# set ENABLE_CONSTRICT dict flag to True / False depending on BMP4 concentration
                    cell, 
                    field = SHH_field,
